...
const INK = Color("443e35")
const SAGE = Color("73856b")
const MUTED = Color("766957")
...
func _ready() -> void:
	...
	theme.set_stylebox("normal", "Button", box(Color("eee8dc"), 20))
	theme.set_stylebox("hover", "Button", box(Color("e6dece"), 20))
	theme.set_stylebox("pressed", "Button", box(Color("d8ddcf"), 20))
	theme.set_stylebox("focus", "Button", box(Color(0,0,0,0), 20, SAGE))
	theme.set_stylebox("disabled","Button",box(Color("e5e0d4"),20))
	theme.set_color("font_disabled_color","Button",Color("9d9587"))
	theme.set_stylebox("normal","LineEdit",box(Color("fffdf8"),16,Color("c7cdbb")))
	theme.set_stylebox("focus","LineEdit",box(Color("fffdf8"),16,SAGE))
...
func box(color: Color, radius := 24, border := Color.TRANSPARENT) -> StyleBoxFlat:
...
func label(text: String, font_size := 24, color := INK, serif := false) -> Label:
	...
	if serif and ResourceLoader.exists("res://assets/Lora.ttf"): node.add_theme_font_override("font", load("res://assets/Lora.ttf"))
	return node
...
func button(text: String, callback: Callable, parent: Node, accent := false) -> Button:
	...
	if accent:
		node.add_theme_stylebox_override("normal", box(SAGE, 20))
...
func show_page(next: String) -> void:
	...
	for child in get_children():
		if child != music and child != effects: remove_child(child); child.queue_free()
	...
	var header := row(shell)
	var brand := label("pocket kin", 35, INK, true)
	brand.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	header.add_child(brand)
	coin_label = label("✦  %d" % World.data.coins, 25, SAGE)
	header.add_child(coin_label)
	var menu := button("• • •", func(): show_page("Settings"), header)
	menu.custom_minimum_size=Vector2(75,55)
	...
	var nav := row(shell, 5)
	for name in ["Home", "Play", "Explore", "Walk", "Room"]:
		var b := button(name, func(): show_page(name), nav, name == page)
		b.add_theme_font_size_override("font_size", 20)
		b.custom_minimum_size.y = 72
	toast_label = label("", 22, Color.WHITE)
	toast_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	toast_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	toast_label.add_theme_stylebox_override("normal", box(INK, 16))
...
func section(kicker: String, title: String, description := "") -> void:
	body.add_child(label(kicker.to_upper(), 18, SAGE))
	body.add_child(label(title, 39, INK, true))
...
func adoption() -> void:
	...
	button("Meet my little friend", func():
		hatch(selected_egg,input.text)
	, hero, true)
...
func spawn_heart(parent: Node, pos: Vector2) -> void:
	...
	tween.tween_property(heart, "position", pos - Vector2(randf_range(-15, 15), 55), 0.65)
...
func bubble_bath_modal() -> void:
	...
	section("Bubble Bath Time", "Splish Splash!", "Tap the warm bubbles to give %s a joyful, squeaky-clean bath." % World.data.pet.name)
	var stage := card(body, Color(0.92, 0.96, 0.95, 1.0))
	...
	bubble_grid.add_theme_constant_override("h_separation", 10)
	bubble_grid.add_theme_constant_override("v_separation", 10)
	...
	for i in range(8):
		...
		b.custom_minimum_size = Vector2(120, 68)
...
func home() -> void:
	...
	title_box.add_child(label("YOUR LITTLE EVERYDAY", 18, SAGE))
	pet_title = label(pet.name + "'s happy place", 37, INK, true)
	...
	if hour >= 20 or hour < 6:
		tint_color = Color("c8d0e8") # twilight starry glow
	elif hour >= 17:
		tint_color = Color("fed9c8") # golden sunset glow
	elif hour < 11:
		tint_color = Color("fef4dc") # warm morning sunrise
	var light_overlay := ColorRect.new()
	light_overlay.color = Color(tint_color.r, tint_color.g, tint_color.b, 0.16)
	...
	# Furnishings share stable room slots and remain behind the companion.
	...
	# Floating Mood Thought Bubble above pet
	var thought := PanelContainer.new()
	thought.add_theme_stylebox_override("panel", box(Color("fffef9"), 14, Color("dcd5c7")))
	thought.position = Vector2(170, 48)
	thought.custom_minimum_size = Vector2(320, 38)
	...
	# Interactive Touch Petting Area (touch & drag strokes emit hearts & purrs)
	var pet_touch := Control.new()
	pet_touch.position = Vector2(191, 100)
	pet_touch.size = Vector2(282, 282)
	var stroke_dist := [0.0]
	pet_touch.gui_input.connect(func(event: InputEvent):
		if event is InputEventScreenDrag or event is InputEventMouseMotion:
			spawn_heart(stage, event.position + pet_touch.position)
			stroke_dist[0] += 12.0
	...
	# Vibrant Needs Meters
	var meters := row(body, 10)
	var meter_colors := {
		"hunger": Color("f38c6c"),
		"happiness": Color("f06292"),
		"cleanliness": Color("4db6ac"),
		"energy": Color("ffb74d")
	}
	for key in ["hunger", "happiness", "cleanliness", "energy"]:
		var panel := card(meters, Color("f9f6ef"))
		panel.get_parent().size_flags_horizontal = Control.SIZE_EXPAND_FILL
		var text := label({"hunger":"🍗 Fed","happiness":"💖 Happy","cleanliness":"🫧 Clean","energy":"⚡ Rested"}[key], 18)
		panel.add_child(text)
		var bar := ProgressBar.new()
		bar.custom_minimum_size = Vector2(102, 11)
		bar.show_percentage = false
		bar.add_theme_stylebox_override("background", box(Color("e5dfd2"), 6))
		bar.add_theme_stylebox_override("fill", box(meter_colors[key], 6))
		panel.add_child(bar)
		needs[key] = bar

	# Action Buttons (Wash opens interactive bubble bath!)
	var actions := row(body, 9)
	button("🍎 Feed", func(): feed_menu(), actions)
	button("💖 Cuddle", func(): do_care("love"), actions)
	button("🫧 Bath", func(): bubble_bath_modal(), actions)
	button("💤 Sleep", func(): do_care("sleep"), actions)

	if pet.ill: button("Feeling poorly · give free treatment",func(): do_care("treat"),body,true)
	var note := card(body,Color("e8eddf"))
	...
	if pet.bond>=30: button("Show me a little trick",perform_trick,note)
...
func hatch(species: int, pet_name: String) -> void:
	...
	if not World.data.settings.reduced_motion:
		var tween:=create_tween()
		for i in range(6): tween.tween_property(egg,"rotation",0.08 if i%2==0 else -0.08,0.18)
...
func feed_menu() -> void:
	show_page("Home")
	for child in body.get_children(): body.remove_child(child);child.queue_free()
	section("Something tasty","A little picnic.","Fresh fruit, always free. Discover what your friend loves.")
	var grid:=GridContainer.new()
	grid.columns=2
...
func do_care(action: String) -> void:
	if World.care(action):
		...
		toast({"feed":"A full tummy and a happy little heart.","love":"Your favorite place is together.","clean":"Fresh, fluffy, and ready for the day.","sleep":"A little rest works wonders.","treat":"Feeling better. Thank you for looking after me."}[action])
...
func refresh() -> void:
	...
	for key in needs:
		if is_instance_valid(needs[key]): needs[key].value = World.data.pet[key]
	if is_instance_valid(pet_image): pet_image.texture = Art.pet(World.data.pet.species,pose())
	if is_instance_valid(mood_label):
		mood_label.text = "%s  ·  Friendship %d  ·  %s" % [str(World.data.pet.stage).capitalize(),World.data.pet.bond,"Dreaming" if World.data.pet.sleeping else ("Needs a little care" if World.data.pet.ill else "Happy to see you")]
...
func play_page() -> void:
	section("Make a little mischief","Play, laugh, repeat.","Short games. Happy hearts. A few petals for your home.")
	for entry in [["catch","Orchard catch","Slide the basket to catch falling peaches."],["match","Memory meadow","Find the matching pairs of little treasures."],["rhythm","Raindrop rhythm","Tap as the ripple meets the ring."]]:
		var content := card(body)
		var header := row(content)
		header.add_child(Art.image(Art.atlas("res://assets/objects-final.png",6,4,[0,7,15][["catch","match","rhythm"].find(entry[0])]),Vector2(90,90)))
		header.add_child(label(entry[1],29,INK,true))
		paragraph(entry[2],content)
		paragraph("Personal best: %d" % World.data.bests.get(entry[0],0),content,19)
		button("Let's play   →",func(): start_game(entry[0]),content,true)
...
func start_game(kind: String) -> void:
	...
	game.finished.connect(func(score: int):
		...
		show_page("Play")
		play_sound("reward")
		toast("Lovely playing! Score %d · +%d petals" % [score,reward])
...
func outing(destination: int) -> void:
	...
	if ResourceLoader.exists("res://assets/"+file+".png"): body.add_child(Art.image(load("res://assets/"+file+".png"),Vector2(0,350)))
	for i in range(3):
		button(["Among the flowers","Beside the path","Under a little leaf"][i],func():
			if Time.get_unix_time_from_system()-last_explore < 15: toast("Let's enjoy this little moment first."); return
...
func walk_page() -> void:
	...
	var content := card(body, Color("e8eddf"))
	...
	progress.add_theme_stylebox_override("fill", box(Color("73856b"), 10))
	content.add_child(progress)

	# Expedition Milestones
	var milestone_card := card(body, Color("f3eee3"))
...
func room_page() -> void:
	...
	for item in World.catalog():
		...
		button("Place / wear" if owned else "✦ %d petals" % item.cost,func():
			if World.buy_equip(item): show_page("Room"); toast("A lovely little choice. See it at home!")
			else: toast("Earn a few more petals through play.")
		,content,owned)
	var premium := card(body,Color("eee7f0"))
...
func album_page() -> void:
	...
	if World.data.memories.is_empty(): paragraph("Your first memory is just around the corner.",body)
	...
	if not World.data.discoveries.is_empty(): paragraph("Your discoveries: " + ", ".join(World.data.discoveries),body)
...
func sanctuary_page() -> void:
	...
	if ResourceLoader.exists("res://assets/sanctuary.png"): body.add_child(Art.image(load("res://assets/sanctuary.png"),Vector2(0,250)))
	...
	if World.data.sanctuary.is_empty(): paragraph("A peaceful garden for your future grown friends.",body)
...
func _process(delta: float) -> void:
	...
	if is_instance_valid(pet_image) and not World.data.settings.reduced_motion:
		pet_image.position.y = 100 + sin(time * 1.8) * 4

func _notification(what: int) -> void:
	if what == NOTIFICATION_APPLICATION_RESUMED:
		World.reconcile()
		if page == "Walk" and World.data.walking.enabled: Platform.call_service("steps",{"activated":World.data.walking.activated})
